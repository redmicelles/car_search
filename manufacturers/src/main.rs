...
impl<'a> Manufacturer<'a> {
    fn description(&self) -> String {
        ...
        let country: &str = self.country.unwrap_or_default();
        format!(
            "\tName: {}\n\tCommon Name: {}\n\tCountry: {}",
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let args: Vec<String> = env::args().collect(); //collect all arguemnets passed to application
    if args.len() < 2 {
        println!("Usage: {} <search term>", args[0]);
        return Ok(());
    }
    let keyword: &String = &args[1];
    //create new reqwest client instance
    ...
    let manufaturers = manufacturer_json.map(|manufacturer_json| {
    ...
    let found_manufacturers = manufaturers
